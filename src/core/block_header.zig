...
pub const BlockHeader = struct {
    ...
    pub fn headerToPayload(header: *BlockHeader) [*]u8 {
        const bytes: [*]u8 = @ptrCast(header);
        return bytes[@sizeOf(BlockHeader)..][0..header.size];
    }

    pub fn payloadToHeader(payload: [*]u8) *BlockHeader {
        const header: [*]u8 = payload - @sizeOf(BlockHeader); // size of payload + header
